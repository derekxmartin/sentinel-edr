/*
 * akesoedr-agent/amsi/amsi_register.cpp
...
{
    WCHAR clsidStr[64];
    GuidToString(AKESOEDR_AMSI_PROVIDER_CLSID, clsidStr, 64);

    std::printf("AkesoEDRAgent: Registering AMSI provider %ls\n", clsidStr);
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: Failed to create AMSI provider key (error %ld)\n",
                    status);
        if (status == ERROR_ACCESS_DENIED) {
            std::printf("AkesoEDRAgent: AMSI registration requires administrator privileges\n");
        }
        return false;
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: Failed to create CLSID key (error %ld)\n",
                    status);
        return false;
    }

    /* Set display name */
    const WCHAR displayName[] = L"AkesoEDR AMSI Provider";
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: Failed to create InprocServer32 key (error %ld)\n",
                    status);
        return false;
    }
    ...
    std::printf("AkesoEDRAgent: AMSI provider registered (DLL: %ls)\n", dllPath);
    return true;
}
...
AmsiProviderUnregister()
{
    ...
    std::printf("AkesoEDRAgent: Unregistering AMSI provider %ls\n", clsidStr);
    ...
    if (status != ERROR_SUCCESS && status != ERROR_FILE_NOT_FOUND) {
        std::printf("AkesoEDRAgent: Failed to delete AMSI provider key (error %ld)\n",
                    status);
    }
    ...
    if (status != ERROR_SUCCESS && status != ERROR_FILE_NOT_FOUND) {
        std::printf("AkesoEDRAgent: Failed to delete CLSID key (error %ld)\n",
                    status);
    }

    std::printf("AkesoEDRAgent: AMSI provider unregistered\n");
    return true;
}
