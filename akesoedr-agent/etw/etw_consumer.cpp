/*
 * akesoedr-agent/etw/etw_consumer.cpp
 ...
 * consumer thread that converts events into AKESOEDR_EVENT telemetry.
 *
 * Architecture:
...
#define ETW_SESSION_NAME    L"AkesoEDREtwSession"
...
EtwConsumerInit()
{
    ...
    std::printf("AkesoEDRAgent: ETW consumer initializing...\n");
...

    {
        ...
        if (status == ERROR_SUCCESS) {
            std::printf("AkesoEDRAgent: Cleaned up stale ETW session\n");
        }
...

    {
        ...
        if (status != ERROR_SUCCESS) {
            std::printf("AkesoEDRAgent: StartTraceW failed (error %lu)\n", status);
            if (status == ERROR_ACCESS_DENIED) {
                std::printf("AkesoEDRAgent: ETW requires administrator privileges\n");
            }
            return false;
        }
    }

    std::printf("AkesoEDRAgent: ETW trace session created (handle=%llu)\n",
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (DotNet) failed (error %lu)\n",
            status);
        /* Non-fatal — session exists, just no DotNet events */
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-DotNETRuntime\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (DNS) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-DNS-Client\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (PowerShell) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-PowerShell\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (Kerberos) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-Security-Kerberos\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (AMSI) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Antimalware-Scan-Interface\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (RPC) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-RPC\n");
    }
    ...
    if (status != ERROR_SUCCESS) {
        std::printf("AkesoEDRAgent: EnableTraceEx2 (Kernel-Process) failed (error %lu)\n",
            status);
    } else {
        std::printf("AkesoEDRAgent: Enabled provider: Microsoft-Windows-Kernel-Process\n");
    }
...
EtwConsumerStart()
{
    ...
    std::printf("AkesoEDRAgent: ETW consumer thread started\n");
}
...
EtwConsumerStop()
{
    ...
    std::printf("AkesoEDRAgent: ETW consumer stopping...\n");
    ...
    /* Stop the trace session — this unblocks ProcessTrace() */
    {
        ...
        if (status != ERROR_SUCCESS && status != ERROR_WMI_INSTANCE_NOT_FOUND) {
            std::printf("AkesoEDRAgent: ControlTrace STOP failed (error %lu)\n",
                status);
        }
    }
    ...
    std::printf("AkesoEDRAgent: ETW consumer stopped\n");
}
...
EtwConsumerThreadFunc()
{
    ...
    if (s_ConsumerHandle == INVALID_PROCESSTRACE_HANDLE) {
        ...
        std::printf("AkesoEDRAgent: OpenTraceW failed (error %lu)\n", err);
        return;
    }
    ...
    if (status != ERROR_SUCCESS && status != ERROR_CANCELLED && status != 1223) {
        std::printf("AkesoEDRAgent: ProcessTrace failed (status=%lu)\n", status);
    }
}
 ...
 * provider parser, then pushes the resulting AKESOEDR_EVENT to the queue.
 *
