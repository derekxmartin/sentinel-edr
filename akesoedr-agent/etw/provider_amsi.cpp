/*
 * akesoedr-agent/etw/provider_amsi.cpp
 ...
 *
 * NOTE: AMSI events route through Payload.Amsi (AkesoEDRSourceAmsi),
 * not Payload.Etw, reusing the existing AKESOEDR_AMSI_EVENT struct.
 *
 ...
 *
 * We map these to our AKESOEDR_AMSI_RESULT enum for consistency.
 */
static AKESOEDR_AMSI_RESULT
MapAmsiResult(LONG rawResult)
{
    if (rawResult == 0)
        return AkesoEDRAmsiClean;
    if (rawResult == 1)
        return AkesoEDRAmsiSuspicious;       /* Not detected = informational */
    if (rawResult >= 32768)
        return AkesoEDRAmsiMalware;
    if (rawResult >= 16384)
        return AkesoEDRAmsiBlocked;
    return AkesoEDRAmsiSuspicious;
}
...
bool
ParseAmsiEvent(PEVENT_RECORD pEvent, AKESOEDR_EVENT* outEvent)
{
    ...
    /* ── Populate the AKESOEDR_EVENT ───────────────────────────────── */

    memset(outEvent, 0, sizeof(AKESOEDR_EVENT));
    ...
    outEvent->Source   = AkesoEDRSourceAmsi;   /* Routes through Payload.Amsi */
    outEvent->Severity = AkesoEDRSeverityInformational;

    outEvent->ProcessCtx.ProcessId = pEvent->EventHeader.ProcessId;

    /* AMSI payload (NOT ETW payload — uses Payload.Amsi) */
    AKESOEDR_AMSI_EVENT* amsi = &outEvent->Payload.Amsi;

    /* Copy AppName */
    size_t appLen = wcslen(appName);
    if (appLen >= AKESOEDR_MAX_PATH) appLen = AKESOEDR_MAX_PATH - 1;
    ...
    if (contentName) {
        ...
        if (cnLen >= AKESOEDR_MAX_RULE_NAME) cnLen = AKESOEDR_MAX_RULE_NAME - 1;
