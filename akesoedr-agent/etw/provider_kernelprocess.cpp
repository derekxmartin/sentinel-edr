/*
 * akesoedr-agent/etw/provider_kernelprocess.cpp
...
bool
ParseKernelProcessEvent(PEVENT_RECORD pEvent, AKESOEDR_EVENT* outEvent)
{
    ...
    if (eventId == KPROC_EVENT_PROCESS_START) {
        ...
        if (ptr < dataEnd) {
            ...
            AnsiToWchar(ansiName, imageName, AKESOEDR_MAX_PATH);
        }
    ...
    outEvent->Source   = AkesoEDRSourceEtw;
    outEvent->Severity = AkesoEDRSeverityInformational;

    outEvent->ProcessCtx.ProcessId = processId;

    /* ETW payload */
    AKESOEDR_ETW_EVENT* etw = &outEvent->Payload.Etw;
    etw->Provider  = AkesoEDREtwKernelProc;
    ...
    if (imgLen >= AKESOEDR_MAX_PATH) imgLen = AKESOEDR_MAX_PATH - 1;
