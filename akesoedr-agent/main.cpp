/*
 * akesoedr-agent/main.cpp
 * User-mode agent service entry point.
 *
 * Two modes:
 *   1. Service mode (default): Registers with the SCM and runs as a
 *      Windows service (SERVICE_WIN32_OWN_PROCESS).
 *   2. Console mode (--console): Runs interactively for debugging.
 *      Ctrl+C triggers graceful shutdown.
 *
 * P4-T1: Service skeleton + event pipeline.
 * P9-T3: Configuration file support (--config).
 */

#include <windows.h>
#include <cstdio>
#include <cstring>
#include "service.h"
#include "config.h"
#include "constants.h"

/* File-scope config — loaded once in main(), read by ServiceMain / RunConsoleMode */
static AkesoEDRConfig g_AgentConfig;

/* Accessor for service.cpp */
const AkesoEDRConfig& GetAgentConfig() { return g_AgentConfig; }

int
main(int argc, char* argv[])
{
    /* Disable stdout buffering so ETW events appear immediately in console */
    setvbuf(stdout, NULL, _IONBF, 0);

    /* Parse command-line arguments */
    bool consoleMode = false;
    const char* configPath = "C:\\AkesoEDR\\akesoedr.conf";

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--console") == 0) {
            consoleMode = true;
        } else if (strcmp(argv[i], "--config") == 0 && i + 1 < argc) {
            configPath = argv[++i];
        }
    }

    /* Load configuration */
    ConfigSetDefaults(g_AgentConfig);
    if (ConfigLoad(g_AgentConfig, configPath)) {
        std::printf("AkesoEDRAgent: Config loaded from %s\n", configPath);
    } else {
        std::printf("AkesoEDRAgent: Config file not found (%s), using defaults\n",
                    configPath);
    }

    if (consoleMode) {
        std::printf("AkesoEDR Agent v%s\n", AKESOEDR_VERSION);
        RunConsoleMode(g_AgentConfig);
        return 0;
    }

    /* Service mode — register with the SCM */
    SERVICE_TABLE_ENTRYW serviceTable[] = {
        { (LPWSTR)AKESOEDR_AGENT_SERVICE, ServiceMain },
        { nullptr, nullptr }
    };

    if (!StartServiceCtrlDispatcherW(serviceTable)) {
        DWORD err = GetLastError();
        if (err == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
            /* Not running as a service — show usage */
            std::printf("AkesoEDR Agent v%s\n", AKESOEDR_VERSION);
            std::printf("Usage:\n");
            std::printf("  akesoedr-agent.exe --console    "
                        "Run interactively (for debugging)\n");
            std::printf("  akesoedr-agent.exe --config <path>  "
                        "Specify config file\n");
            std::printf("  sc create AkesoEDRAgent binPath= "
                        "\"<path>\\akesoedr-agent.exe\"\n");
            std::printf("  sc start AkesoEDRAgent           "
                        "Start as a Windows service\n");
            return 1;
        }
        std::fprintf(stderr, "StartServiceCtrlDispatcher failed: %lu\n", err);
        return 1;
    }

    return 0;
}
