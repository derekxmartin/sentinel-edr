/*
 * akesoedr-agent/rules/rule_parser.cpp
...
RuleParser::ParseSeverity(const std::string& sev)
{
    ...
    if (lower == "informational" || lower == "info")
        return AkesoEDRSeverityInformational;
    if (lower == "low")
        return AkesoEDRSeverityLow;
    if (lower == "medium" || lower == "med")
        return AkesoEDRSeverityMedium;
    if (lower == "high")
        return AkesoEDRSeverityHigh;
    if (lower == "critical" || lower == "crit")
        return AkesoEDRSeverityCritical;
    return AkesoEDRSeverityMedium;  /* Default */
}
...
RuleParser::ParseSource(const std::string& src)
{
    ...
    if (lower == "driverprocess")   return AkesoEDRSourceDriverProcess;
    if (lower == "driverthread")    return AkesoEDRSourceDriverThread;
    if (lower == "driverobject")    return AkesoEDRSourceDriverObject;
    if (lower == "driverimageload") return AkesoEDRSourceDriverImageLoad;
    if (lower == "driverregistry")  return AkesoEDRSourceDriverRegistry;
    if (lower == "driverminifilter") return AkesoEDRSourceDriverMinifilter;
    if (lower == "drivernetwork")   return AkesoEDRSourceDriverNetwork;
    if (lower == "hookdll")         return AkesoEDRSourceHookDll;
    if (lower == "etw")             return AkesoEDRSourceEtw;
    if (lower == "amsi")            return AkesoEDRSourceAmsi;
    if (lower == "scanner")         return AkesoEDRSourceScanner;
    if (lower == "selfprotect")     return AkesoEDRSourceSelfProtect;
    return AkesoEDRSourceMax;       /* Invalid */
}
...
{
    rule.severity = AkesoEDRSeverityMedium;
