/*
 * akesoedr-agent/rules/sequence_engine.cpp
...
SequenceEngine::Init(const std::string& rulesDir)
{
    ...
    std::printf("AkesoEDRAgent: Loaded %zu sequence rule(s)\n",
                m_rules.size());
...
void
SequenceEngine::Evaluate(const AKESOEDR_EVENT& evt,
...
bool
SequenceEngine::StepMatches(const AKESOEDR_EVENT& evt,
