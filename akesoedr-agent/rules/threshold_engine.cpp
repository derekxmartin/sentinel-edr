/*
 * akesoedr-agent/rules/threshold_engine.cpp
...
ThresholdEngine::Init(const std::string& rulesDir)
{
    ...
    std::printf("AkesoEDRAgent: Loaded %zu threshold rule(s)\n",
                m_rules.size());
...
void
ThresholdEngine::Evaluate(const AKESOEDR_EVENT& evt,
...
bool
ThresholdEngine::MatchesFilter(const AKESOEDR_EVENT& evt,
