/*
 * akesoedr-agent/scanner/memory_scanner.cpp
...
bool
MemoryScanner::ScanProcess(ULONG targetPid, AKESOEDR_EVENT& alertOut)
{
    if (!IsReady()) {
        return false;
    }
            ...
            if (ok && bytesRead > 0) {
                ...
                if (m_scanner->ScanBuffer(buffer.data(), bytesRead, result)) {
                    if (result.IsMatch) {
                        ...
                        scan.ScanType      = AkesoEDRScanMemory;
                        scan.IsMatch       = TRUE;
                        scan.TargetProcessId = targetPid;

                        /* Format address range in TargetPath */
                        _snwprintf_s(scan.TargetPath, AKESOEDR_MAX_PATH,
                                     _TRUNCATE,
