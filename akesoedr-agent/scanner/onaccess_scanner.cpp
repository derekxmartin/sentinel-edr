/*
 * akesoedr-agent/scanner/onaccess_scanner.cpp
...
bool
OnAccessScanner::OnFileEvent(const AKESOEDR_FILE_EVENT& fileEvt,
                              AKESOEDR_EVENT& alertOut)
{
    ...
    if (IsCacheValid(hashKey)) {
        ...
        if (cached.isMatch) {
            ...
            AkesoEDREventInit(&alertOut, AkesoEDRSourceScanner,
                              AkesoEDRSeverityHigh);
            ...
            WCHAR win32Path[AKESOEDR_MAX_PATH];
            if (NtPathToWin32(fileEvt.FilePath, win32Path, AKESOEDR_MAX_PATH)) {
                wcscpy_s(scan.TargetPath, AKESOEDR_MAX_PATH, win32Path);
            } else {
                wcscpy_s(scan.TargetPath, AKESOEDR_MAX_PATH, fileEvt.FilePath);
            }
     ...
     */
    WCHAR scanPath[AKESOEDR_MAX_PATH];
    const WCHAR* pathToScan = fileEvt.FilePath;

    if (NtPathToWin32(fileEvt.FilePath, scanPath, AKESOEDR_MAX_PATH)) {
        pathToScan = scanPath;
    }

    if (!m_scanner->ScanFile(pathToScan, AkesoEDRScanOnAccess, result)) {
