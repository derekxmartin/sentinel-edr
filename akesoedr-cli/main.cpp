/*
 * akesoedr-cli/main.cpp
 * Console management tool for AkesoEDR.
 *
 * Communicates with the running akesoedr-agent over
 * \\.\pipe\AkesoEDRCommand using the IPC protocol defined in ipc.h.
 *
 * Subcommands:
...
PrintUsage()
{
    std::printf(
        "AkesoEDR CLI v1.0.0\n"
        "\n"
        "Usage: akesoedr-cli <command> [args] [--json]\n"
        "\n"
        "Commands:\n"
...
ConnectToAgent()
{
    ...
    AkesoEDRIpcBuildHandshake(&hs, AkesoEDRClientCli,
                              GetCurrentProcessId(), 1);

    BYTE    hsBuf[128];
    UINT32  hsBytes = 0;
    if (AkesoEDRIpcWriteFrame(hsBuf, sizeof(hsBuf),
            &hs, sizeof(hs), &hsBytes) != AkesoEDRSerializeOk) {
        CloseHandle(hPipe);
        return INVALID_HANDLE_VALUE;
    }
    ...
    if (bytesRead >= sizeof(UINT32) + sizeof(AKESOEDR_IPC_HANDSHAKE_REPLY)) {
        auto* reply = reinterpret_cast<AKESOEDR_IPC_HANDSHAKE_REPLY*>(
            replyBuf + sizeof(UINT32));
        if (reply->Status != AkesoEDRHandshakeOk) {
 ...
 */
static std::string
SendCommand(HANDLE hPipe, AKESOEDR_CMD_TYPE cmdType, const wchar_t* arg)
{
    /* Build command message */
    AKESOEDR_IPC_COMMAND cmd = {};
    UINT32 payloadAfterHdr = sizeof(cmd) - sizeof(AKESOEDR_IPC_HEADER);
    AkesoEDRIpcHeaderInit(&cmd.Header, AkesoEDRMsgCommand,
                          payloadAfterHdr, 2);
    cmd.CommandType = (UINT32)cmdType;

    if (arg && arg[0] != L'\0') {
        wcsncpy_s(cmd.Argument, AKESOEDR_CMD_MAX_ARG, arg, _TRUNCATE);
    }

    /* Frame and send */
    BYTE    sendBuf[sizeof(UINT32) + sizeof(cmd)];
    UINT32  sendBytes = 0;
    if (AkesoEDRIpcWriteFrame(sendBuf, sizeof(sendBuf),
            &cmd, sizeof(cmd), &sendBytes) != AkesoEDRSerializeOk) {
        return "";
    }
    ...
    /* Read reply: [UINT32 frame_len] [AKESOEDR_IPC_COMMAND_REPLY] [JSON] */
    BYTE    recvBuf[AKESOEDR_CMD_MAX_REPLY + 256];
    DWORD   bytesRead = 0;
    ...
    if (bytesRead < sizeof(UINT32) + sizeof(AKESOEDR_IPC_COMMAND_REPLY)) {
        std::fprintf(stderr, "Error: Reply too short\n");
        return "";
    }

    auto* replyHdr = reinterpret_cast<AKESOEDR_IPC_COMMAND_REPLY*>(
        recvBuf + sizeof(UINT32));

    if (AkesoEDRIpcHeaderValidate(&replyHdr->Header) != AkesoEDRSerializeOk) {
    ...
    const char* jsonData = reinterpret_cast<const char*>(
        recvBuf + sizeof(UINT32) + sizeof(AKESOEDR_IPC_COMMAND_REPLY));

    if (jsonSize > 0 && sizeof(UINT32) + sizeof(AKESOEDR_IPC_COMMAND_REPLY) + jsonSize <= bytesRead) {
...
{
    std::printf("AkesoEDR Agent Status\n");
    std::printf("========================\n");
...
DoRulesUpdate(bool jsonOutput)
{
    ...
    const std::string rulesDir     = "C:\\AkesoEDR\\rules";
    const std::string yaraRulesDir = "C:\\AkesoEDR\\yara-rules";
    ...
    for (auto& repo : repos) {
        if (!IsGitRepo(repo.dir)) {
            std::fprintf(stderr,
                ...
                "Run 'akesoedr-cli rules update --init "
    ...
    std::string json = SendCommand(hPipe, AkesoEDRCmdRulesUpdate, nullptr);
    CloseHandle(hPipe);
...
DoRulesInit(int argc, char* argv[], bool jsonOutput)
{
    ...
    if (rulesRepoUrl.empty() || yaraRepoUrl.empty()) {
        std::fprintf(stderr,
            ...
            "Usage: akesoedr-cli rules update --init "
...
PrintConfig(const std::string& json)
{
    ...
    std::printf("AkesoEDR Agent Configuration\n");
    std::printf("================================\n");
...
int main(int argc, char* argv[])
{
    ...
    AKESOEDR_CMD_TYPE cmdType;
    wchar_t argument[AKESOEDR_CMD_MAX_ARG] = {};

    if (strcmp(command, "status") == 0) {
        cmdType = AkesoEDRCmdStatus;

    } else if (strcmp(command, "alerts") == 0) {
        cmdType = AkesoEDRCmdAlerts;
        /* Optional count argument */
        if (argc >= 3 && argv[2][0] != '-') {
            MultiByteToWideChar(CP_UTF8, 0, argv[2], -1,
                                argument, AKESOEDR_CMD_MAX_ARG);
        }

    } else if (strcmp(command, "scan") == 0) {
        cmdType = AkesoEDRCmdScan;
        if (argc < 3 || argv[2][0] == '-') {
            std::fprintf(stderr, "Error: scan requires a file path\n");
            std::fprintf(stderr, "Usage: akesoedr-cli scan <path>\n");
            return 1;
        }
        MultiByteToWideChar(CP_UTF8, 0, argv[2], -1,
                            argument, AKESOEDR_CMD_MAX_ARG);
        ...
        cmdType = AkesoEDRCmdRulesReload;

    } else if (strcmp(command, "connections") == 0) {
        cmdType = AkesoEDRCmdConnections;

    } else if (strcmp(command, "processes") == 0) {
        cmdType = AkesoEDRCmdProcesses;

    } else if (strcmp(command, "hooks") == 0) {
        cmdType = AkesoEDRCmdHooks;

    } else if (strcmp(command, "config") == 0) {
        cmdType = AkesoEDRCmdConfig;
    ...
    if (jsonOutput) {
        ...
        switch (cmdType) {
        case AkesoEDRCmdStatus:       PrintStatus(json);       break;
        case AkesoEDRCmdAlerts:       PrintAlerts(json);       break;
        case AkesoEDRCmdScan:         PrintScanResult(json);   break;
        case AkesoEDRCmdRulesReload:  PrintRulesReload(json);  break;
        case AkesoEDRCmdConnections: PrintConnections(json);  break;
        case AkesoEDRCmdProcesses:   PrintProcesses(json);    break;
        case AkesoEDRCmdHooks:       PrintHooks(json);        break;
        case AkesoEDRCmdConfig:      PrintConfig(json);       break;
