/*
 * akesoedr-drv/callbacks_object.c
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, AkesoEDRObjectCallbackInit)
#pragma alloc_text(PAGE, AkesoEDRObjectCallbackStop)
#endif
...
AkesoEDRObjectPreCallback(
    _In_    PVOID                          RegistrationContext,
    _Inout_ POB_PRE_OPERATION_INFORMATION  OperationInfo
);

static BOOLEAN
AkesoEDRIsProtectedProcess(
    _In_ PEPROCESS Process
);

static VOID
AkesoEDRFillProcessCtxForObject(
    _Out_ AKESOEDR_PROCESS_CTX* Ctx,
    _In_  HANDLE                ProcessId
);

/* ── AkesoEDRObjectCallbackInit ─────────────────────────────────────────── */
...
AkesoEDRObjectCallbackInit(VOID)
{
    ...
    RtlInitUnicodeString(&altitude, AKESOEDR_OB_ALTITUDE);
    ...
    opReg[0].PreOperation = AkesoEDRObjectPreCallback;
    ...
    opReg[1].PreOperation = AkesoEDRObjectPreCallback;
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: ObRegisterCallbacks failed 0x%08X\n", status));
        return status;
    }
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: Object handle callbacks registered (altitude %wZ)\n",
        &altitude));
...
AkesoEDRObjectCallbackStop(VOID)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: Object handle callbacks unregistered\n"));
}
...
)
{
    ...
    if (!AkesoEDRIsProtectedProcess(targetProcess)) {
        return OB_PREOP_SUCCESS;
    }
    ...
    __try {
        ...
        event->Source   = AkesoEDRSourceDriverObject;
        event->Severity = AkesoEDRSeverityMedium;

        /* Fill process context (the SOURCE — who is opening the handle) */
        AkesoEDRFillProcessCtxForObject(&event->ProcessCtx, sourcePid);

        /* Fill object event payload */
        event->Payload.Object.Operation =
            (OperationInfo->Operation == OB_OPERATION_HANDLE_CREATE)
                ? AkesoEDRObjOpCreate
                : AkesoEDRObjOpDuplicate;

        event->Payload.Object.ObjectType =
            isProcessType ? AkesoEDRObjTypeProcess : AkesoEDRObjTypeThread;
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_TRACE_LEVEL,
            "AkesoEDR: Object %s %s PID=%lu -> PID=%lu Access=0x%X\n",
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: Exception 0x%08X in object callback PID=%lu -> PID=%lu\n",
            GetExceptionCode(),
            (ULONG)(ULONG_PTR)sourcePid,
            (ULONG)(ULONG_PTR)targetPid));
    }
