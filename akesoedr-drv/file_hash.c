 ...
 *   4. Emits a AKESOEDR_EVENT with the hash and file metadata
 *
 * This keeps the I/O path non-blocking — the work item runs at
 * PASSIVE_LEVEL on a system worker thread.
 *
 * Files larger than AKESOEDR_SCAN_MAX_FILE_SIZE (50 MB) are skipped
 * with HashSkipped = TRUE.
 *
 * IRQL:
 *   - AkesoEDRFileHashQueueWorkItem: <= APC_LEVEL (post-op callback)
...
typedef struct _AKESOEDR_HASH_WORK_CTX {
    ...
    GUID                 EventId;
} AKESOEDR_HASH_WORK_CTX;

/* ── Forward declarations ───────────────────────────────────────────────── */

static VOID
AkesoEDRFileHashWorkItemCb(
    _In_     PDEVICE_OBJECT DeviceObject,
    _In_opt_ PVOID          Context
);

static NTSTATUS
AkesoEDRFileHashComputeSha256(
...
AkesoEDRFileHashInit(VOID)
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: BCryptOpenAlgorithmProvider failed 0x%08X\n",
            status));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: File hash subsystem initialized (SHA-256)\n"));

    return STATUS_SUCCESS;
}

VOID
AkesoEDRFileHashStop(VOID)
{
    ...
    while (InterlockedCompareExchange(&g_HashWorkItemsActive, 0, 0) > 0) {
        ...
        if (++spins > 50) {     /* 5 seconds max */
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: Timed out waiting for %ld hash work items\n",
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: File hash subsystem stopped\n"));
}
...
{
    AKESOEDR_HASH_WORK_CTX *ctx = NULL;
    NTSTATUS                status;
    ...
    ctx = (AKESOEDR_HASH_WORK_CTX *)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
        sizeof(AKESOEDR_HASH_WORK_CTX),
        AKESOEDR_TAG_HASH
    );
    if (!ctx) {
        goto fallback;
    ...
    /* Process context */
    __try {
        AkesoEDRMinifilterFillProcessCtx(&ctx->ProcessCtx, Data);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        /* Non-fatal */
    }
...
fallback_free:
    ...
    if (ctx) {
        ExFreePoolWithTag(ctx, AKESOEDR_TAG_HASH);
    }

fallback:
    /* Fall back to synchronous event emission without hash */
    AkesoEDRMinifilterEmitFileEvent(Data, FltObjects, Operation);
}
...
{
    AKESOEDR_HASH_WORK_CTX *ctx = (AKESOEDR_HASH_WORK_CTX *)Context;
    ...
    if (ctx->FilePath[0] != L'\0' && ctx->Instance) {
        __try {
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "AkesoEDR: Exception 0x%08X in hash computation\n",
                GetExceptionCode()));
        }
    }
...
)
{
    ...
    *HashSkipped = FALSE;
     ...
     */
    if (FileSize->QuadPart > AKESOEDR_SCAN_MAX_FILE_SIZE) {
        *HashSkipped = TRUE;
        return STATUS_SUCCESS;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
            "AkesoEDR: FltCreateFileEx failed 0x%08X for hash\n", status));
        return status;
    }
        ...
        if (NT_SUCCESS(status)) {
            *FileSize = stdInfo.EndOfFile;
            ...
            if (FileSize->QuadPart == 0) {
                RtlStringCchCopyA(Sha256Hex, AKESOEDR_MAX_HASH_HEX,
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: BCryptCreateHash failed 0x%08X\n", status));
        goto cleanup;
    }
    ...
    while (bytesRemaining > 0) {
        ...
        if (!NT_SUCCESS(status)) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "AkesoEDR: BCryptHashData failed 0x%08X\n", status));
            goto cleanup;
        }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: BCryptFinishHash failed 0x%08X\n", status));
        goto cleanup;
    }

    /* Convert to hex string */
    AkesoEDRHashBytesToHex(rawHash, SHA256_HASH_SIZE, Sha256Hex);
...
cleanup:
    ...
    if (readBuffer) {
        ExFreePoolWithTag(readBuffer, AKESOEDR_TAG_HASH);
    }
    if (fileObject) {
