 ...
 * process initialization), causing LdrLoadDll to load akesoedr-hook.dll.
 *
...
/*
 * Path to akesoedr-hook.dll on the target VM.
 * Must be deployed to this location before injection.
 */
#define AKESOEDR_HOOK_DLL_PATH  L"C:\\AkesoEDR\\akesoedr-hook.dll"
...
static BOOLEAN
AkesoEDRIsExcludedProcess(
    _In_ PEPROCESS Process
);

static BOOLEAN
AkesoEDRIsAlreadyInjected(
    _In_ HANDLE ProcessId
);

static VOID
AkesoEDRRecordInjection(
    _In_ HANDLE ProcessId
);

static PVOID
AkesoEDRResolveLdrLoadDll(
    _In_ PVOID NtdllBase
);

static PVOID
AkesoEDRAllocateAndWriteShellcode(
    _In_ PVOID  LdrLoadDllAddr,
    _In_ PVOID  ShellcodeRegionBase
);

static VOID NTAPI
AkesoEDRKapcKernelRoutine(
    _In_    PKAPC           Apc,
    _Inout_ PKNORMAL_ROUTINE *NormalRoutine,
    _Inout_ PVOID           *NormalContext,
    _Inout_ PVOID           *SystemArgument1,
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, AkesoEDRKapcInjectInit)
#pragma alloc_text(PAGE, AkesoEDRKapcInjectStop)
#endif
...
AkesoEDRKapcInjectInit(VOID)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: KAPC injection initialized (hook DLL: %ls)\n",
        AKESOEDR_HOOK_DLL_PATH));
...
AkesoEDRKapcInjectStop(VOID)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: KAPC injection stopped\n"));
}
...
)
{
    ...
    __try {
        ...
        if (AkesoEDRIsExcludedProcess(process)) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_TRACE_LEVEL,
                "AkesoEDR: KAPC skip excluded PID=%lu\n",
        ...
        if (AkesoEDRIsAlreadyInjected(ProcessId)) {
            __leave;
        }
         ...
         */
        {
            PVOID ntdllBase = AkesoEDRLookupNtdllBase(ProcessId);
            if (!ntdllBase) {
                KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                    "AkesoEDR: No saved ntdll base for PID=%lu\n",
                    (ULONG)(ULONG_PTR)ProcessId));
                __leave;
            }

            ldrLoadDll = AkesoEDRResolveLdrLoadDll(ntdllBase);
        }

        if (!ldrLoadDll) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: Failed to resolve LdrLoadDll PID=%lu\n",
                (ULONG)(ULONG_PTR)ProcessId));
            __leave;
        }

        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
            "AkesoEDR: LdrLoadDll resolved at 0x%p for PID=%lu\n",
        ...
        if (!NT_SUCCESS(status)) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: ZwAllocateVirtualMemory failed 0x%08X PID=%lu\n",
         ...
         */
        shellcodeEntry = AkesoEDRAllocateAndWriteShellcode(
            ldrLoadDll, shellcodeRegion);

        if (!shellcodeEntry) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: Shellcode write failed PID=%lu\n",
                (ULONG)(ULONG_PTR)ProcessId));
        ...
        kapc = (PKAPC)ExAllocatePool2(
            POOL_FLAG_NON_PAGED, KAPC_SIZE, AKESOEDR_TAG_KAPC);
        if (!kapc) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: KAPC allocation failed PID=%lu\n",
                (ULONG)(ULONG_PTR)ProcessId));
            __leave;
        }
        ...
        KeInitializeApc(
            ...
            OriginalApcEnvironment,
            AkesoEDRKapcKernelRoutine,      /* Frees KAPC on delivery */
            AkesoEDRKapcRundownRoutine,     /* Frees KAPC if thread dies */
        ...
        if (!KeInsertQueueApc(kapc, NULL, NULL, 0)) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                "AkesoEDR: KeInsertQueueApc failed PID=%lu\n",
                (ULONG)(ULONG_PTR)ProcessId));
            ExFreePoolWithTag(kapc, AKESOEDR_TAG_KAPC);
            __leave;
        }
        ...
        AkesoEDRRecordInjection(ProcessId);

        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
            "AkesoEDR: KAPC queued for PID=%lu (shellcode=0x%p, LdrLoadDll=0x%p)\n",
            (ULONG)(ULONG_PTR)ProcessId, shellcodeEntry, ldrLoadDll));

    } __except (EXCEPTION_EXECUTE_HANDLER) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: Exception 0x%08X in KAPC injection PID=%lu\n",
...
)
{
    ...
    __try {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: Exception 0x%08X resolving LdrLoadDll\n",
            GetExceptionCode()));
    }
 ...
 *   [0x010] L"\\??\\C:\\AkesoEDR\\akesoedr-hook.dll\0"
 *   [0x200] x64 shellcode:
...
)
{
    ...
    static const WCHAR dllPath[] = AKESOEDR_HOOK_DLL_PATH;
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
            "AkesoEDR: ZwProtectVirtualMemory failed 0x%08X\n", status));
...
)
{
    ...
    ExFreePoolWithTag(Apc, AKESOEDR_TAG_KAPC);
}
