/*
 * akesoedr-drv/minifilter_pipes.c
 ...
 * checking the file path, then emit AKESOEDR_PIPE_EVENT telemetry.
 *
...
)
{
    ...
    event->Source   = AkesoEDRSourceDriverPipe;
    ...
    __try {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: Exception 0x%08X in pipe FillProcessCtx\n",
            GetExceptionCode()));
    }
    ...
    __try {
        ...
        if (NT_SUCCESS(status)) {
            ...
            RtlStringCchCopyNW(
                ...
                AKESOEDR_MAX_PIPE_NAME,
                nameInfo->Name.Buffer,
                nameInfo->Name.Length / sizeof(WCHAR)
            );

            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "AkesoEDR: Pipe name resolved: %wZ\n",
                &nameInfo->Name));
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "AkesoEDR: FltGetFileNameInformation failed for pipe: 0x%08X\n",
                status));
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: Exception 0x%08X in pipe name query\n",
            GetExceptionCode()));
    }
    ...
    /* Send to agent */
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
        "AkesoEDR: Sending pipe event src=%d pid=%lu pipe=%S\n",
        event->Source,
        ...
        if (PipeName->Length >= prefix->Length) {
            ...
            if (RtlCompareUnicodeString(&namePrefix, prefix, TRUE) == 0) {
                KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
                    "AkesoEDR: Suspicious pipe detected: %wZ\n", PipeName));
                return TRUE;
            }
        }
    }
...
)
{
    ...
    /*
     * Do NOT use AkesoEDRMinifilterShouldSkipPreOp here — it filters
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
        "AkesoEDR: PreCreateNamedPipe hit! reqMode=%d pid=%lu\n",
...
)
{
    ...
    if (!NT_SUCCESS(Data->IoStatus.Status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: PostCreateNamedPipe FAILED status=0x%08X\n",
            Data->IoStatus.Status));
        return FLT_POSTOP_FINISHED_PROCESSING;
    }

    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
        "AkesoEDR: PostCreateNamedPipe SUCCESS — emitting pipe event\n"));
