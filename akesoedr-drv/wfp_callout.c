 ...
 * PID) and emit AKESOEDR_NETWORK_EVENT telemetry via the filter comms
 * port. All traffic is always permitted (FWP_ACTION_CONTINUE).
 *
 * IRQL: ALE classify functions run at PASSIVE_LEVEL, so AkesoEDRCommsSend
 * (which calls FltSendMessage at <= APC_LEVEL) can be invoked directly.
 *
 * Rate limiting: A per-PID hash table limits events to
 * AKESOEDR_NET_MAX_EVENTS_PER_SEC per process. A timer DPC resets
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(INIT, AkesoEDRWfpInit)
#pragma alloc_text(PAGE, AkesoEDRWfpStop)
#endif
...
)
{
    ...
    return (count > AKESOEDR_NET_MAX_EVENTS_PER_SEC) ? TRUE : FALSE;
}
 ...
 * work item that calls AkesoEDRCommsSend at PASSIVE_LEVEL.
 *
...
)
{
    ...
    if (ctx) {
        AkesoEDRCommsSend(&ctx->Event);
        IoFreeWorkItem(ctx->WorkItem);
        ExFreePoolWithTag(ctx, AKESOEDR_TAG_NETWORK);
    }
}
 ...
 * Extracts connection metadata and emits a AKESOEDR_NETWORK_EVENT.
 *
...
WfpClassifyCommon(
    ...
    _In_ AKESOEDR_NET_DIRECTION                direction,
...
)
{
    ...
    if (irql <= PASSIVE_LEVEL) {
        ...
        event->Source   = AkesoEDRSourceDriverNetwork;
        ...
        ExFreePoolWithTag(event, AKESOEDR_TAG_NETWORK);
        ...
        ctx->Event.Source   = AkesoEDRSourceDriverNetwork;
        ctx->Event.Severity = AkesoEDRSeverityInformational;
...
)
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmEngineOpen0 failed 0x%08X\n", status));
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmTransactionBegin0 failed 0x%08X\n", status));
        goto cleanup_engine;
    }

    /* ── Step 3: Register + add outbound callout ──────────────────────── */

    sCallout.calloutKey           = AKESOEDR_WFP_CALLOUT_CONNECT_V4;
    sCallout.classifyFn           = AkesoEDRClassifyConnect;
    sCallout.notifyFn             = AkesoEDRNotifyConnect;
    sCallout.flowDeleteFn         = NULL;

    status = FwpsCalloutRegister0(DeviceObject, &sCallout, &s_ConnectCalloutId);
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpsCalloutRegister0 (connect) failed 0x%08X\n", status));
        goto abort_txn;
    }

    displayData.name        = L"AkesoEDR ALE Connect v4";
    displayData.description = L"Monitors outbound IPv4 connections";

    mCallout.calloutKey       = AKESOEDR_WFP_CALLOUT_CONNECT_V4;
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmCalloutAdd0 (connect) failed 0x%08X\n", status));
        goto abort_txn;
    }
    ...
    sCallout.calloutKey           = AKESOEDR_WFP_CALLOUT_RECV_V4;
    sCallout.classifyFn           = AkesoEDRClassifyRecv;
    sCallout.notifyFn             = AkesoEDRNotifyRecv;
    sCallout.flowDeleteFn         = NULL;

    status = FwpsCalloutRegister0(DeviceObject, &sCallout, &s_RecvCalloutId);
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpsCalloutRegister0 (recv) failed 0x%08X\n", status));
        goto abort_txn;
    }

    displayData.name        = L"AkesoEDR ALE Recv/Accept v4";
    displayData.description = L"Monitors inbound IPv4 connections";

    RtlZeroMemory(&mCallout, sizeof(mCallout));
    mCallout.calloutKey       = AKESOEDR_WFP_CALLOUT_RECV_V4;
    mCallout.displayData      = displayData;
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmCalloutAdd0 (recv) failed 0x%08X\n", status));
        goto abort_txn;
    }

    /* ── Step 5: Add sublayer ─────────────────────────────────────────── */

    subLayer.subLayerKey         = AKESOEDR_WFP_SUBLAYER_GUID;
    displayData.name             = L"AkesoEDR Sublayer";
    displayData.description      = L"AkesoEDR network inspection sublayer";
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmSubLayerAdd0 failed 0x%08X\n", status));
        goto abort_txn;
    }
    ...
    filter.filterKey            = AKESOEDR_WFP_FILTER_CONNECT_V4;
    displayData.name            = L"AkesoEDR Connect Filter v4";
    displayData.description     = L"Inspection filter for outbound IPv4";
    filter.displayData          = displayData;
    filter.layerKey             = FWPM_LAYER_ALE_AUTH_CONNECT_V4;
    filter.subLayerKey          = AKESOEDR_WFP_SUBLAYER_GUID;
    filter.action.type          = FWP_ACTION_CALLOUT_INSPECTION;
    filter.action.calloutKey    = AKESOEDR_WFP_CALLOUT_CONNECT_V4;
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmFilterAdd0 (connect) failed 0x%08X\n", status));
        goto abort_txn;
    }
    ...
    RtlZeroMemory(&filter, sizeof(filter));
    filter.filterKey            = AKESOEDR_WFP_FILTER_RECV_V4;
    displayData.name            = L"AkesoEDR Recv Filter v4";
    displayData.description     = L"Inspection filter for inbound IPv4";
    filter.displayData          = displayData;
    filter.layerKey             = FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4;
    filter.subLayerKey          = AKESOEDR_WFP_SUBLAYER_GUID;
    filter.action.type          = FWP_ACTION_CALLOUT_INSPECTION;
    filter.action.calloutKey    = AKESOEDR_WFP_CALLOUT_RECV_V4;
    filter.weight.type          = FWP_EMPTY;
    filter.numFilterConditions  = 0;    /* Match all traffic */
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmFilterAdd0 (recv) failed 0x%08X\n", status));
        goto abort_txn;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "AkesoEDR: FwpmTransactionCommit0 failed 0x%08X\n", status));
        goto cleanup_engine;
    }
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "AkesoEDR: WFP callouts registered (connect=%u, recv=%u)\n",
