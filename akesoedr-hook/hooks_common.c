...
void AkesoEDRHooksSetReady(void) { g_HooksReady = TRUE; }
BOOL AkesoEDRHooksAreReady(void) { return g_HooksReady; }
...
void
AkesoEDRTlsInit(void)
{
    g_TlsIndex = TlsAlloc();
}

void
AkesoEDRTlsCleanup(void)
{
...
const char *
AkesoEDRHookFunctionName(AKESOEDR_HOOK_FUNCTION func)
{
    if (func >= 0 && func < AkesoEDRHookMax) {
...
ULONG
AkesoEDRGetTargetPid(HANDLE ProcessHandle)
{
 ...
 *
 * This function is loader-lock safe — AkesoEDRPipeClientPush uses only
 * InterlockedIncrement, memcpy, GetSystemTimePreciseAsFileTime, SetEvent.
 */
void
AkesoEDREmitHookEvent(AKESOEDR_HOOK_EVENT *evt)
{
    AkesoEDRPipeClientPush(evt);
}
