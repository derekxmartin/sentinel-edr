...
#ifndef AKESOEDR_HOOKS_COMMON_H
#define AKESOEDR_HOOKS_COMMON_H

#include <windows.h>
#include "telemetry.h"

/*
 * AkesoEDRGetTargetPid
 *   Resolve a process handle to a PID. Returns 0 if the handle refers
 *   to the current process (or on failure).
 */
ULONG AkesoEDRGetTargetPid(HANDLE ProcessHandle);

/*
 * AkesoEDRGetCallingModule
...
/*
 * AkesoEDREmitHookEvent
 *   Push a hook event into the pipe client ring buffer.
 *   The background worker thread serializes and sends to the agent.
 */
void AkesoEDREmitHookEvent(AKESOEDR_HOOK_EVENT *evt);

/*
 * AkesoEDRHookFunctionName
 *   Return a human-readable name for a AKESOEDR_HOOK_FUNCTION enum value.
 */
const char *AkesoEDRHookFunctionName(AKESOEDR_HOOK_FUNCTION func);

/*
 * AkesoEDRHooksSetReady / AkesoEDRHooksAreReady
 ...
 */
void AkesoEDRHooksSetReady(void);
BOOL AkesoEDRHooksAreReady(void);

/*
 * AkesoEDRTlsInit / AkesoEDRTlsCleanup
 *   Allocate/free the manual TLS index used by the reentrancy guard.
 *   Call from DllMain PROCESS_ATTACH / PROCESS_DETACH.
 */
void AkesoEDRTlsInit(void);
void AkesoEDRTlsCleanup(void);

/*
 * AkesoEDRCaptureStackHash
 ...
 */
ULONG AkesoEDRCaptureStackHash(void);

/*
 * AkesoEDREnterHook / AkesoEDRLeaveHook
 ...
 *     return status;
 */
BOOL AkesoEDREnterHook(void);
void AkesoEDRLeaveHook(void);
