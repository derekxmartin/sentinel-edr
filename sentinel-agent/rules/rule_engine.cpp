/*
 * sentinel-agent/rules/rule_engine.cpp
...
RuleEngine::Init(const std::string& rulesDir)
{
    ...
    std::printf("SentinelAgent: Loaded %zu detection rule(s)\n",
                m_rules.size());
...
void
RuleEngine::Evaluate(const SENTINEL_EVENT& evt,
...
bool
RuleEngine::MatchesRule(const SENTINEL_EVENT& evt,
...
bool
RuleEngine::EvaluateCondition(const SENTINEL_EVENT& evt,
...
std::string
RuleEngine::ResolveField(const SENTINEL_EVENT& evt,
                         ...
                         ProcessTable& processTable)
{
    ...
    if (evt.Source == SentinelSourceDriverThread) {
    ...
    if (evt.Source == SentinelSourceDriverObject) {
    ...
    if (evt.Source == SentinelSourceDriverImageLoad) {
    ...
    if (evt.Source == SentinelSourceDriverRegistry) {
