/*
 * sentinel-amsi/amsi_provider.cpp
...
SentinelAmsiProvider::QueryInterface(REFIID riid, void** ppv)
{
...
SentinelAmsiProvider::AddRef()
{
    return InterlockedIncrement(&m_refCount);
}

ULONG STDMETHODCALLTYPE
SentinelAmsiProvider::Release()
{
...
HRESULT STDMETHODCALLTYPE
SentinelAmsiProvider::Scan(IAmsiStream* stream, AMSI_RESULT* result)
{
...
SentinelAmsiProvider::CloseSession(ULONGLONG session)
{
...
HRESULT STDMETHODCALLTYPE
SentinelAmsiProvider::DisplayName(LPWSTR* displayName)
{
...
void
SentinelAmsiProvider::LoadSignatures()
{
 ...
 */
bool
SentinelAmsiProvider::MatchContent(const BYTE* data, ULONG size)
{
...
HRESULT STDMETHODCALLTYPE
SentinelAmsiClassFactory::QueryInterface(REFIID riid, void** ppv)
{
    if (!ppv) return E_POINTER;
...
ULONG STDMETHODCALLTYPE
SentinelAmsiClassFactory::AddRef()
{
    return InterlockedIncrement(&m_refCount);
}

ULONG STDMETHODCALLTYPE
SentinelAmsiClassFactory::Release()
{
    LONG ref = InterlockedDecrement(&m_refCount);
    if (ref == 0) {
        delete this;
...
HRESULT STDMETHODCALLTYPE
SentinelAmsiClassFactory::CreateInstance(IUnknown* pOuter, REFIID riid, void** ppv)
{
    if (pOuter) return CLASS_E_NOAGGREGATION;
    if (!ppv) return E_POINTER;

    auto* provider = new (std::nothrow) SentinelAmsiProvider();
...
HRESULT STDMETHODCALLTYPE
SentinelAmsiClassFactory::LockServer(BOOL lock)
{
    if (lock) {
        InterlockedIncrement(&g_DllRefCount);
    } else {
