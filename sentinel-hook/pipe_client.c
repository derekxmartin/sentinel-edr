 ...
 * SENTINEL_EVENT envelopes, serializes them, and sends over the named pipe
 * to the agent (\\.\pipe\SentinelTelemetry).
 *
...
typedef struct _SENTINEL_RING_ENTRY {
    SENTINEL_HOOK_EVENT     Event;
    LARGE_INTEGER           Timestamp;
    volatile LONG           Ready;      /* 0=empty, 1=written and readable */
} SENTINEL_RING_ENTRY;

static SENTINEL_RING_ENTRY  g_RingBuffer[SENTINEL_HOOK_RING_BUFFER_SIZE];
...
static SENTINEL_PROCESS_CTX g_ProcessCtx;
...
void
SentinelPipeClientShutdown(void)
{
 ...
 */
void
SentinelPipeClientPush(SENTINEL_HOOK_EVENT *evt)
{
    LONG slot;
    SENTINEL_RING_ENTRY *entry;
    FILETIME ft;
    ...
    slot = slot % SENTINEL_HOOK_RING_BUFFER_SIZE;
...
CacheProcessContext(void)
{
    ...
    /* Image path */
    GetModuleFileNameW(NULL, g_ProcessCtx.ImagePath, SENTINEL_MAX_PATH);
    ...
    /* Command line — GetCommandLineW returns a pointer to static buffer */
    {
        ...
        if (cmdLine != NULL) {
            _snwprintf_s(g_ProcessCtx.CommandLine,
                         SENTINEL_MAX_CMDLINE, _TRUNCATE,
                         L"%s", cmdLine);
        }
    }
            ...
            if (GetTokenInformation(hToken, TokenUser,
                ...
                if (ConvertSidToStringSidW(pUser->User.Sid, &sidStr)) {
                    _snwprintf_s(g_ProcessCtx.UserSid,
                                 SENTINEL_MAX_SID_STRING, _TRUNCATE,
                                 L"%s", sidStr);
                    LocalFree(sidStr);
                }
            }
...
{
    SENTINEL_IPC_HANDSHAKE      handshake;
    SENTINEL_IPC_HANDSHAKE_REPLY reply;
    BYTE frameBuf[256];
    UINT32 bytesWritten = 0;
    DWORD dwWritten = 0, dwRead = 0;
    SENTINEL_SERIALIZE_STATUS serStatus;

    /* Build handshake message */
    SentinelIpcBuildHandshake(
        &handshake,
        SentinelClientHookDll,
        GetCurrentProcessId(),
        (*pSequenceNum)++);

    /* Frame it */
    serStatus = SentinelIpcWriteFrame(
        frameBuf, sizeof(frameBuf),
        &handshake, sizeof(handshake),
        &bytesWritten);
    if (serStatus != SentinelSerializeOk) {
        return FALSE;
    }
    ...
    /* Read reply frame: [4-byte length][SENTINEL_IPC_HANDSHAKE_REPLY] */
    {
        BYTE replyBuf[256];
        UINT32 frameLen = 0;

        if (!ReadFile(hPipe, replyBuf, sizeof(replyBuf), &dwRead, NULL)
            || dwRead < sizeof(UINT32) + sizeof(SENTINEL_IPC_HANDSHAKE_REPLY)) {
            return FALSE;
        }

        frameLen = *(UINT32 *)replyBuf;
        if (frameLen < sizeof(SENTINEL_IPC_HANDSHAKE_REPLY)) {
            return FALSE;
        }

        memcpy(&reply, replyBuf + sizeof(UINT32), sizeof(reply));

        if (SentinelIpcHeaderValidate(&reply.Header) != SentinelSerializeOk) {
            return FALSE;
        }

        if (reply.Header.Type != SentinelMsgHandshakeReply) {
            return FALSE;
        }

        if (reply.Status != (UINT32)SentinelHandshakeOk) {
            return FALSE;
        }
    }
...
/*
 * Drain all ready entries from the ring buffer, wrap in SENTINEL_EVENT,
...
DrainRingBuffer(HANDLE hPipe, UINT32 *pSequenceNum)
{
    ...
    for (drained = 0; drained < SENTINEL_HOOK_RING_BUFFER_SIZE; drained++) {
        SENTINEL_RING_ENTRY *entry;
        SENTINEL_EVENT       envelope;
        BYTE                 sendBuf[sizeof(UINT32) + sizeof(SENTINEL_IPC_EVENT_MSG) + sizeof(SENTINEL_EVENT)];
        UINT32               bytesWritten = 0;
        DWORD                dwWritten = 0;

        tail = g_RingTail % SENTINEL_HOOK_RING_BUFFER_SIZE;
        ...
        /* Build SENTINEL_EVENT envelope */
        SentinelEventInit(&envelope, SentinelSourceHookDll,
        ...
        /* Serialize and send */
        if (SentinelIpcSerializeEvent(sendBuf, sizeof(sendBuf),
                                       &envelope, (*pSequenceNum)++,
                                       &bytesWritten) == SentinelSerializeOk) {
...
shutdown:
    ...
    if (hPipe != INVALID_HANDLE_VALUE) {
        ...
        SentinelIpcHeaderInit(
            &disc.Header,
            SentinelMsgDisconnect,
            sizeof(SENTINEL_IPC_DISCONNECT) - sizeof(SENTINEL_IPC_HEADER),
            sequenceNum++);
        disc.Reason = 0;  /* Normal disconnect */

        if (SentinelIpcWriteFrame(discBuf, sizeof(discBuf),
                                   &disc, sizeof(disc),
                                   &bytesWritten) == SentinelSerializeOk) {
